...
def rewrite_article_with_ai(article_data):
    ...
    prompt = f"""You are a professional automotive journalist and expert SEO content writer. Write a highly detailed, deeply engaging, and 100% original news article based on the following snippet.
    ...
    CRITICAL INSTRUCTIONS TO PREVENT DUPLICATE CONTENT PENALTIES:
    1. DO NOT USE ANY TEMPLATES OR BOILERPLATE TEXT. Every sentence must be uniquely written for this specific news topic.
    2. NEVER use phrases like "The practical question for an owner is simple: does this change affect service access", "In this article, we will explore", or "OBD2HQ is tracking".
    3. The content must read like a premium automotive magazine article. 
    4. Translate perfectly and naturally into English, Turkish, German, Spanish, and French. Do not do literal word-for-word translations.
    5. The article must be long-form (at least 4-5 rich paragraphs).
    
    You must output a strictly valid JSON object matching this structure exactly (No Markdown wrappers, just JSON):
    {{
        "id": "a-unique-id",
    ...
    Make the "content" fields contain the long-form detailed paragraphs separated by double newlines (\\n\\n). 
    """
