...
const CODE_FOCUS: Record<string, TopImpressionCodeFocus> = {
  ...
  P0235: {
    query: 'p0235 suzuki',
    title: 'P0235 Code: Turbo Boost Sensor A Circuit Diagnosis',
    answer: 'P0235 points to the turbocharger boost sensor A circuit or pressure signal being outside the expected range. The useful diagnosis is not just the sensor name: compare boost/MAP pressure data, 5V reference, ground, signal voltage, connector condition, intake leaks, wastegate control and related overboost or underboost codes before buying parts.',
    severity: 'Moderate to high when the vehicle has limp mode, low power, black smoke, overboost, underboost or a flashing warning light.',
    safeToDrive: 'Avoid heavy throttle until boost pressure behavior is verified. Stop driving if the engine loses power suddenly, smokes heavily, overheats or enters severe limp mode.',
    firstChecks: [
      'Save freeze-frame data showing RPM, load, throttle position, boost or MAP pressure and intake air temperature.',
      'Compare boost/MAP sensor key-on engine-off reading with atmospheric pressure.',
      'Check 5V reference, ground and signal voltage at the boost pressure sensor connector.',
      'Inspect sensor connector tension, corrosion, oil contamination and harness rub points.',
      'Check intake hoses, intercooler pipes, vacuum lines and wastegate or actuator plumbing for leaks or incorrect routing.',
      'Review related codes such as P0234, P0243, P0299, P0106 and P0108 before replacing the turbo or sensor.',
    ],
    doNotReplace: 'Do not replace the turbocharger or boost sensor first. A bad connector, wiring fault, pressure hose leak, wastegate control issue or incorrect sensor signal can trigger the same P0235 complaint.',
    costLevel: 'Low to moderate when wiring, connector, hose or sensor work solves it; expensive only if actuator or turbo control failure is verified.',
    relatedSearches: ['p0235 suzuki', 'suzuki jimny p0235', 'p0235 turbo boost sensor', 'p0235 symptoms', 'p0235 causes'],
    links: [
      { href: '/en/suzuki/jimny/p0235', label: 'Suzuki Jimny P0235' },
      { href: '/en/codes/p0234', label: 'P0234 overboost code' },
      { href: '/en/codes/p0243', label: 'P0243 wastegate solenoid' },
