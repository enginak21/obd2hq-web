...
const validRoutes = {
  validMakes: cars.map(c => c.make),
  validModels: cars.reduce((acc, car) => {
    acc[car.make] = car.models;
    return acc;
  }, {} as Record<string, string[]>),
  validCodes: Object.keys(baseCodes).map(c => c.toUpperCase())
};

fs.writeFileSync(path.join(__dirname, 'valid_routes.json'), JSON.stringify(validRoutes, null, 2));
