    ...
    '@graph': [
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name, item: `https://obd2hq.com/${locale}/vehicles/${make}/${model}` },
        ],
      },
      {
        '@type': 'ItemList',
        ...
        itemListElement: variants.slice(0, 50).map((variant, index) => ({
          ...
          url: `https://obd2hq.com/${locale}/vehicles/${make}/${model}/${variant.year}/${variant.slug}`,
        })),
      },
      {
        '@type': 'FAQPage',
...
function getVehicleLabels(locale: string): VehicleLabels {
  if (locale === 'tr') {
    return {
      metaTitle: (name: string) => `${name} motor kodlarÄ±, yaÄŸ kapasitesi ve kronik sorunlar`,
      coverage: 'Kapsam',
      generations: 'YÄ±l ve jenerasyon haritasÄ±',
      engineVariants: 'Motor kodlarÄ± ve varyantlar',
      transmissionVariants: 'ÅanzÄ±man varyantlarÄ±',
      sparkPlugs: 'Buji aralÄ±ÄŸÄ±',
      diagnosticCodes: 'Ä°lgili arÄ±za kodlarÄ±',
      commonFailurePatterns: 'YaygÄ±n arÄ±za desenleri',
      serviceNote: 'Servis notu',
      affectedYears: 'Etkilenen yÄ±llar',
      symptoms: 'Belirtiler',
      firstChecks: 'Ä°lk kontroller',
      related: 'Ä°lgili',
      dataQuality: 'Veri kalitesi ve doÄŸrulama notu',
      yearTrimSelector: 'YÄ±l ve kasa seÃ§imi',
      yearTrimSelectorDesc: 'Net motor, yaÄŸ ve servis bilgisi iÃ§in yÄ±lÄ± ve kasayÄ± seÃ§in.',
      verifiedVariants: 'doÄŸrulanmÄ±ÅŸ varyant',
      openVariant: 'DetayÄ± aÃ§',
      engineCodesByYear: 'YÄ±la gÃ¶re motor kodlarÄ±',
      oilCapacityByEngine: 'Motora gÃ¶re yaÄŸ kapasitesi',
      year: 'YÄ±l',
      trim: 'Kasa/versiyon',
      engineCodes: 'Motor kodlarÄ±',
      displacement: 'Motor hacmi',
      fuelSystem: 'YakÄ±t sistemi',
      identifyEngineCode: 'Motor kodunu nasÄ±l doÄŸrularsÄ±nÄ±z?',
      identifyVin: 'Ruhsat, VIN etiketi veya Ã¼retici servis kaydÄ± motor seÃ§eneÄŸini doÄŸrulamak iÃ§in en gÃ¼venli baÅŸlangÄ±Ã§tÄ±r.',
      identifySticker: 'Motor bÃ¶lmesindeki etiketler, emisyon etiketi ve servis geÃ§miÅŸi motor ailesini doÄŸrulamaya yardÄ±mcÄ± olur.',
      identifyScanTool: 'CanlÄ± veri okuyabilen bir OBD2 cihazÄ± motor kontrol Ã¼nitesi ve ilgili kodlarÄ± kontrol etmek iÃ§in kullanÄ±labilir.',
      faqTitle: 'SÄ±k sorulan sorular',
      verifyExactVariant: 'Kesin bilgi iÃ§in yÄ±l, motor, pazar ve kasa seÃ§eneÄŸi birlikte doÄŸrulanmalÄ±dÄ±r.',
      engineCodeQuestion: (name: string) => `${name} motor kodlarÄ± nelerdir?`,
      engineCodeAnswer: (name: string, codes: string) => `${name} iÃ§in rehberde gÃ¶rÃ¼nen baÅŸlÄ±ca motor kodlarÄ±: ${codes}. Kesin motor kodunu yÄ±l ve kasa seÃ§imiyle doÄŸrulayÄ±n.`,
      oilQuestion: (name: string) => `${name} hangi motor yaÄŸÄ±nÄ± kullanÄ±r?`,
      oilAnswer: (name: string, oils: string) => `${name} yaÄŸ bilgisi motor seÃ§eneÄŸine gÃ¶re deÄŸiÅŸir. Rehberdeki Ã¶zet: ${oils}.`,
      problemQuestion: (name: string) => `${name} iÃ§in sÄ±k gÃ¶rÃ¼len sorunlar nelerdir?`,
      problemAnswer: (name: string, problems: string) => `${name} sayfasÄ±nda Ã¶ne Ã§Ä±kan kontroller: ${problems}. ArÄ±za kodu varsa ilgili OBD2 rehberine geÃ§in.`,
    };
  }
  if (locale === 'de') {
    return {
      ...getVehicleLabels('en'),
      metaTitle: (name: string) => `${name} Motorcodes, Ã–lmenge und hÃ¤ufige Probleme`,
      coverage: 'Abdeckung',
      engineCodesByYear: 'Motorcodes nach Jahr',
      oilCapacityByEngine: 'Ã–lmenge nach Motor',
      year: 'Jahr',
      trim: 'Variante',
      engineCodes: 'Motorcodes',
      displacement: 'Hubraum',
      fuelSystem: 'Kraftstoffsystem',
      identifyEngineCode: 'Wie Sie den Motorcode prÃ¼fen',
  ...
  if (locale === 'es') {
    return {
      ...getVehicleLabels('en'),
      metaTitle: (name: string) => `${name} cÃ³digos de motor, aceite y problemas comunes`,
      coverage: 'Cobertura',
      engineCodesByYear: 'CÃ³digos de motor por aÃ±o',
      oilCapacityByEngine: 'Capacidad de aceite por motor',
      year: 'AÃ±o',
      trim: 'VersiÃ³n',
      engineCodes: 'CÃ³digos de motor',
      displacement: 'Cilindrada',
      fuelSystem: 'Sistema de combustible',
      identifyEngineCode: 'CÃ³mo confirmar el cÃ³digo de motor',
  ...
  if (locale === 'fr') {
    return {
      ...getVehicleLabels('en'),
      metaTitle: (name: string) => `${name} codes moteur, huile et problÃ¨mes frÃ©quents`,
      coverage: 'Couverture',
      engineCodesByYear: 'Codes moteur par annÃ©e',
      oilCapacityByEngine: 'CapacitÃ© dâ€™huile par moteur',
      year: 'AnnÃ©e',
      trim: 'Version',
      engineCodes: 'Codes moteur',
      displacement: 'CylindrÃ©e',
      fuelSystem: 'SystÃ¨me carburant',
