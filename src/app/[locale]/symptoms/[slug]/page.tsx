...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  return {
    title: `${localized.title} - Causes, Codes, Safety and First Checks`,
...
export default async function SymptomPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  const faq = [
    {
      q: `Is ${localized.title.toLowerCase()} safe to drive with?`,
      a: localized.driveAdvice,
    },
    {
      q: `Which OBD2 codes are related to ${localized.title.toLowerCase()}?`,
    ...
    <main className="min-h-screen bg-[#0a0f1c] text-slate-200 pb-24">
      ...
      <section className="border-b border-white/5 bg-[#0d1425]">
        <div className="max-w-6xl mx-auto px-6 py-14">
          <nav className="text-sm text-slate-500 mb-6">
            ...
            <Link href={`/${locale}/symptoms`} className="hover:text-white">Symptoms</Link>
          </nav>
      ...
      <section className="max-w-6xl mx-auto px-6 py-12 grid lg:grid-cols-[1fr_340px] gap-8">
        <div className="space-y-6">
          <InfoBlock icon={<Activity className="w-5 h-5" />} title="Most likely causes" items={symptom.likelyCauses} />
          <InfoBlock icon={<Wrench className="w-5 h-5" />} title="First checks before replacing parts" items={symptom.firstChecks} />
          <InfoBlock icon={<Gauge className="w-5 h-5" />} title="Related systems" items={symptom.relatedSystems} />
        </div>

        <aside className="space-y-6">
          <div className="rounded-3xl border border-white/5 bg-[#131b2f] p-6">
            <h2 className="text-sm font-black uppercase tracking-widest text-slate-400 mb-4">Related OBD2 codes</h2>
          ...
          <div className="rounded-3xl border border-white/5 bg-[#131b2f] p-6">
            <h2 className="text-sm font-black uppercase tracking-widest text-slate-400 mb-4">Search intents covered</h2>
          ...
          <Link href={`/${locale}/tools/diagnostic-assistant`} className="flex items-center justify-between rounded-3xl border border-green-400/20 bg-green-500/10 p-6 text-green-100 hover:bg-green-500/15">
            <span className="font-black">Open diagnostic assistant</span>
