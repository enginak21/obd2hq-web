...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  if (!engine) return {};
  return {
    title: `${engine.manufacturer} ${engine.family} Engine Specs, Oil, Problems and Codes`,
    description: `${engine.family} engine database: specs, oil type, oil capacity, timing drive, common failures, maintenance and OBD2 codes.`,
    ...
    <main className="min-h-screen bg-[#0a0f1c] text-slate-200 pb-24">
      <section className="border-b border-white/5 bg-[#0d1425]">
        <div className="max-w-6xl mx-auto px-6 py-14">
          <Link href={`/${locale}/engines`} className="text-sm text-slate-500 hover:text-white">Engine database</Link>
      ...
      <section className="max-w-6xl mx-auto px-6 py-12 grid lg:grid-cols-[1fr_340px] gap-8">
        <div className="space-y-6">
          <Panel title="Specifications" items={[`Induction: ${engine.induction}`, `Timing drive: ${engine.timingDrive}`, `Oil type: ${engine.oilType}`, `Oil capacity: ${engine.oilCapacity}`]} />
          <Panel title="Common failures" items={engine.commonFailures} />
          <Panel title="Maintenance focus" items={engine.maintenance} />
        </div>
        <aside className="space-y-6">
          <Panel title="Reliability score" items={[engine.reliabilityScore]} />
