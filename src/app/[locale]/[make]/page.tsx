...
export default async function MakeDirectoryPage({ params }: PageProps) {
  ...
  const guideTitle = locale === 'tr' ? `${capMake} sayfasini nasil kullanmalisiniz?` : locale === 'de' ? `So nutzen Sie die ${capMake}-Seite` : locale === 'es' ? `Como usar la pagina de ${capMake}` : locale === 'fr' ? `Comment utiliser la page ${capMake}` : `How to use the ${capMake} hub`;
  const guideText = locale === 'tr'
    ? `${capMake} icin once modelinizi secin, ardindan ariza kodu, gosterge isigi veya belirti rehberine ilerleyin. Kod sayfalari parca degistirmeden once yapilacak kontrolleri, guvenli surus uyarilarini, ilgili OBD2 kodlarini ve tahmini masraf seviyesini birlikte verir. Modelinizden emin degilseniz ruhsat, servis kaydi veya VIN destekli parca kataloguyla dogrulama yapin.`
    : locale === 'de'
      ? `Waehlen Sie zuerst Ihr ${capMake}-Modell und oeffnen Sie danach den passenden Fehlercode, die Warnleuchte oder den Symptomleitfaden. Die Code-Seiten zeigen Pruefungen vor dem Teiletausch, Sicherheitshinweise, verwandte OBD2-Codes und eine grobe Kostenstufe. Wenn das Modell unsicher ist, pruefen Sie es ueber Fahrzeugpapiere, Serviceunterlagen oder einen VIN-basierten Teilekatalog.`
      : locale === 'es'
        ? `Elige primero tu modelo ${capMake} y despues abre el codigo, la luz del tablero o la guia de sintomas. Las paginas de codigos reunen pruebas antes de cambiar piezas, avisos de seguridad, codigos OBD2 relacionados y nivel aproximado de coste. Si no estas seguro del modelo, confirmalo con documentacion, historial de servicio o catalogo por VIN.`
        : locale === 'fr'
          ? `Choisissez d'abord votre modele ${capMake}, puis ouvrez le code defaut, le voyant ou le guide de symptomes. Les pages de code regroupent les controles avant remplacement, les alertes de securite, les codes OBD2 lies et un niveau de cout estime. En cas de doute, confirmez le modele avec les papiers, l'entretien ou un catalogue par VIN.`
